...
try:
    ...
    logger.info("dynamo.connect: Utilizing cupy to enable GPU acceleration.")
except ImportError:
    try:
        import numpy as array_module

        logger.warning("dynamo.connect: Failed to load cupy for GPU acceleration, utilizing numpy to provide CPU based operations.")
...
class ActiveOperation(AbstractOperation):
    ...
    def __init__(
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created local NIXL transfer descriptors: {self._local_xfer_descs}")
        self._remote_xfer_descs = self._connector._nixl.get_xfer_descs(
            descs=self._remote_dlist,
            mem_type=str(self._remote_memtype),
        )
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created remote NIXL transfer descriptors: {self._remote_xfer_descs}")
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created NIXL transfer handle: {self._xfer_hndl}")
    ...
    def _release(self) -> None:
        ...
        if self._xfer_hndl is not None:
            try:
                logger.debug(f"dynamo.connect.{self.__class__.__name__}: NIXL transfer handle {self._xfer_hndl} released.")
                self._connector._nixl.release_xfer_handle(self._xfer_hndl)
            except Exception as e:
                logger.error(f"dynamo.connect.{self.__class__.__name__}: Failed to release resources: {e}")
                error = e
            finally:
        ...
        try:
            ...
            logger.error(f"dynamo.connect.{self.__class__.__name__}: Failed to release base class resources: {e}")
    ...
    def _cancel_(self) -> None:
        ...
        logger.info(f"dynamo.connect.{self.__class__.__name__}: Cancellation requested for operation {{ kind={self._operation_kind}, remote='{self._remote.name}', status={self._status} }}.")
    ...
    async def _wait_for_completion_(self) -> None:
        ...
        while True:
            if iteration_count & 10 == 0:
                logger.debug(f"dynamo.connect.{self.__class__.__name__}: Waiting for operation {{ kind={self._operation_kind}, remote='{self._remote.name}', duration={iteration_count / 10}s }}.")
            match self.status:
    ...
    def status(self) -> OperationStatus:
        ...
        if self._status == OperationStatus.UNINITIALIZED:
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: NIXL reported transfer state: {state}")
            if state == "ERR":
        ...
        if self._status != old_status:
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: {{ remote: '{self._remote.name}' status: '{old_status}' => '{self._status}' }}.")
...
class Connector:
    ...
    def __init__(
        ...
        runtime : Optional[DistributedRuntime], optional
            Reference the dynamo runtime used by the component, attempts to use the current runtime when `None`.
        ...
        ------
        TypeError
            When `namespace` is provided and not of type 'str'.
        TypeError
            When `runtime` iis provided and not of type `dynamo.runtime.DistributedRuntime`.
        TypeError
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()}.")
    ...
    async def initialize(self) -> None:
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Initialized {{ name: '{self._worker_id}', namespace '{self._namespace}' }} completed.")
...
class Descriptor:
    ...
    def __init__(
        ...
        if isinstance(data, torch.Tensor):
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()} from `torch.Tensor`.")
        ...
        # Data is `tuple[ndarray, Device]`.
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()} from `tuple[ndarray, Device|str]`.")
        ...
        # Data is `bytes`.
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()} from `bytes`.")
        ...
        # Data is `tuple[int, int, Device, dtype, tuple, Any]`.
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()} from `tuple[int, int, Device|str, Any]`.")
        else:
            raise TypeError(TYPE_ERROR_MESSAGE)
    ...
    def __del__(self) -> None:
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Deleted {self.__repr__()}.")
    ...
    def register_memory(
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Registered {self.__repr__()} with NIXL.")
...
class PassiveOperation(AbstractOperation):
    ...
    def metadata(self) -> RdmaMetadata:
        ...
        if self._serialized_request is None:
            ...
            if isinstance(self._local_descriptors, list):
                descriptors = [desc.metadata() for desc in self._local_descriptors]
            else:
                descriptors = [self._local_descriptors.metadata()]
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: Compressed NIXL metadata from {original_len} bytes to {compressed_len} bytes.")
            if compressed_len > original_len:
                logger.warning(f"dynamo.connect.{self.__class__.__name__}: Compressed NIXL metadata is larger than original ({compressed_len} > {original_len}).")
    ...
    def status(self) -> OperationStatus:
        ...
        if isinstance(notifications, dict):
            ...
            logger.debug(f"dynamo.connect.{self.__class__.__name__}: NIXL reported notifications: {len(notifications)}.")
            ...
            if remote_state == OperationStatus.COMPLETE:
                ...
                logger.debug(f"dynamo.connect.{self.__class__.__name__}: {{ remote: '{self._connector.name}' status: '{old_status}' => '{self._status}' }}.")
...
class ReadOperation(ActiveOperation):
    ...
    def __init__(
        ...
        if not isinstance(remote_metadata, RdmaMetadata):
            raise TypeError("Argument `remote_metadata` must be `dynamo.connect.RdmaMetadata`.")
        if remote_metadata.operation_kind != OperationKind.READ.value:
        ...
        super().__init__(remote, OperationKind.READ, local_descriptors, remote_descriptors, remote_metadata.notification_key)
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()}")
...
class WriteOperation(ActiveOperation):
    ...
    def __init__(
        ...
        TypeError
            When `remote_metadata` is not a `dynamo.connect.RdmaMetadata`.
        ValueError
        ...
        super().__init__(remote, OperationKind.WRITE, local_descriptors, remote_descriptors, remote_metadata.notification_key)
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Created {self.__repr__()}")
